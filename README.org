...
'{ 
   ;; primitives
     ...
     :hashed #{:set :map},
     :atom #{:number :function :keyword :symbol :string :map-entry},
     :coll #{:seq :vec :set :map},
     :prim #{:number :function :seq :vec :keyword :symbol :string :map-entry :nil :set :map}}
#+end_src
...
;;=>
 '{:nil #{nil}
   ...
   :seq #{IndexedSeq LazySeq PersistentTreeMapSeq NodeSeq PersistentArrayMapSeq ES6IteratorSeq ChunkedSeq
  ...
  :word #{:keyword :symbol :string},
  :atom #{:number :function :keyword :symbol :string :map-entry},
  :prim #{:number :fun :seq :vec :keyword :symbol :string :map-entry :nil :set :map}}
#+end_src

Those type registry are certainly not complete but I think that's enough
to see the point.

Along with the definitions of those registries, the =nemesis.types=
...
(t/parentof #{:string :symbol} :symbol) ;=> #{:sym :str}

(t/parentof :word #{:string :symbol}) ;=> :word

(t/parentof :word #{:string :vec}) ;=> false
